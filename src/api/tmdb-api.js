...
export const getUpcomingMovies = (args) => {
  ...
  const { pageNumber } = idPart
  return fetch(
    `https://api.themoviedb.org/3/movie/upcoming?api_key=${import.meta.env.VITE_TMDB_KEY}&language=en-US&include_adult=false&include_video=false&page=${pageNumber}`
  )
  .then((res) => {
    if (!res.ok) {
      throw new Error (res.json().message);
